    ...
    try:
        while time.time() - start_time < duration:
            ...
            if stats:
                ...
                print(f"Sample: CPU {stats['cpu_percent']:.1f}%, RAM {stats['memory_mb']:.1f}MB, Children: {stats['children_count']}")
...
def test_python_wrapper():
    ...
    return monitor_process_overhead([
        'python3', 'process_monitor_pty.py', 
        '--interval', '2', '--', 'sleep', '30'
    ], duration=10)
...
def test_bash_wrapper():
    ...
    return monitor_process_overhead(['./process_monitor_wrapper.sh', 'sleep', '30'], duration=10)
...
def compare_results(baseline, python_normal, bash, python_high_freq):
    ...
    print(f"{'Test':<25} {'Avg CPU%':<10} {'Max CPU%':<10} {'Avg RAM MB':<12} {'Max RAM MB':<12}")
    print("-" * 70)
    
    for name, result in tests:
        if 'error' not in result:
            print(f"{name:<25} {result['avg_cpu']:<10.1f} {result['max_cpu']:<10.1f} "
...
def main():
    ...
    # Check dependencies
    if not os.path.exists('process_monitor_pty.py'):
        print("ERROR: process_monitor_pty.py not found")
        sys.exit(1)
