...
def main():
    ...
    tests = [
        (
            ["../toptle.py", "--interval", "2", "--", "sleep", "20"],
            "Python Wrapper (2s)",
        ),
        (
            ["../toptle.py", "--interval", "0.5", "--", "sleep", "20"],
            "Python Wrapper (0.5s)",
        ),
    ...
    for cmd, name in tests:
        ...
        if "error" not in result:
            print(
                f"Summary - Avg CPU: {result['avg_cpu']:.1f}%, Avg RAM: {result['avg_memory']:.1f}MB"
            )
        else:
    ...
    for name, result in results:
        if "error" not in result:
            print(
                f"{name:<20} {result['avg_cpu']:<10.1f} {result['max_cpu']:<10.1f} {result['avg_memory']:<12.1f}"
            )
        else:
    ...
    if baseline and python_2s:
        ...
        print(
            f"Python wrapper overhead: +{cpu_overhead:.1f}% CPU, +{ram_overhead:.1f}MB RAM"
        )
